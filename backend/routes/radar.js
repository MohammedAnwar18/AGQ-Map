...
router.get('/intel', async (req, res) => {
    res.json({
        ...
        incidents: [
            { type: "اشتباك مسلح", city: "جنين - الضفة", text: "اشتباكات مسلحة بين مقاومين وقوات الاحتلال إثر عملية اقتحام واسعة للمخيم.", lat: 32.4611, lon: 35.2975 },
            { type: "غارة جوية", city: "قطاع غزة - رفح", text: "طيران الاستطلاع يستهدف منزلاً سكنياً وسط مدينة رفح ووقوع إصابات حرجة في المنطقة.", lat: 31.2801, lon: 34.2504 },
            { type: "دفاع جوي", city: "أصفهان - إيران", text: "انفجارات تسمع في سماء أصفهان وتفعيل منظومات الدفاع الجوي لصد هجوم بطائرات مسيرة مجهولة.", lat: 32.6539, lon: 51.6660 },
            { type: "قصف مدفعي", city: "جنوب لبنان", text: "قصف مدفعي عنيف يستهدف أطراف بلدة عيتا الشعب في القطاع الأوسط من جنوب لبنان.", lat: 33.1091, lon: 35.3353 },
            { type: "استهداف سفن", city: "البحر الأحمر - الحديدة", text: "تقارير أولية عن استهداف سفينة شحن دولية قبالة سواحل الحديدة ضمن عمليات التضييق البحري.", lat: 14.79, lon: 42.95 }
        ]
    });
});
...
router.get('/telegram', async (req, res) => {
    try {
        const tgramData = [
            { channel: 'أخبار غزة (عاجل)', text: "عاجل: غارات جوية صهيونية عنيفة وحزام ناري يستهدف مناطق متفرقة في خانيونس وجنوب قطاع غزة.", date: new Date().toISOString() },
            { channel: 'التلفزيون الإيراني', text: "قيادة الحرس الثوري تؤكد: أي اعتداء عسكري على منشآتنا الاستراتيجية سيُقابل برد زلزالي ومباشر لن يتخيله العدو.", date: new Date(Date.now() - 150000).toISOString() },
            { channel: 'الإعلام الحربي - لبنان', text: "المقاومة الإسلامية تطلق رشقة صاروخية مكثفة نحو الشمال رداً على استهداف المدنيين والقرى الآمنة في الجنوب.", date: new Date(Date.now() - 300000).toISOString() },
            { channel: 'مراسل الضفة', text: "متابعة أمنية: إغلاق حواجز محيط نابلس ودخول تعزيزات عسكرية باتجاه بلدة حوارة إثر بلاغ عن عملية إطلاق نار فدائية.", date: new Date(Date.now() - 500000).toISOString() },
