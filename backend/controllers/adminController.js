...
const toggleShopStatus = async (req, res) => {
    try {
        const { shopId } = req.params;
        await pool.query('UPDATE shops SET is_hidden = NOT COALESCE(is_hidden, false) WHERE id = $1', [shopId]);
        res.json({ message: 'Shop status toggled' });
    } catch (e) {
...
const toggleShopLock = async (req, res) => {
    try {
        const { shopId } = req.params;
        await pool.query('UPDATE shops SET is_locked = NOT COALESCE(is_locked, false) WHERE id = $1', [shopId]);
        res.json({ message: 'Shop lock toggled' });
    } catch (e) {
