...
// اتّسعت لتحتضن المدينة (٤٢٢ م) وهامشاً من الريف حولها.
export const TERRAIN_SPAN = 480;

// عدد العقد على كل ضلع. ١٦١ عقدة = ١٦٠ خليّة = ثلاثة أمتار للخليّة.
//
// الخليّة المترّان كانت أدقّ، لكنها على هذه المساحة تعني ستّة
// وخمسين ألف رأس تُرفع إلى كرت الرسم في كل إطار أثناء النحت. ثلاثة
// أمتار تكفي حفرةً وطلعةً وتُبقي الشبكة في ستّة وعشرين ألفاً.
...
export const flattenArea = (x, z, size, levelOverride) => {
    // القلب يتجاوز البلاطة بخليّة كاملة.
    //
    // الارتفاع بين العقد مُستوفى: عقدة على حافّة البلاطة بالضبط
    // تُسطَّح، لكن جارتها خارجها تكون قد بدأت تذوب — فيرتفع السطح
    // داخل البلاطة عند حافّتها. وخليّة الشبكة ثلاثة أمتار والبلاطة
    // ثمانية، فالخطأ يبلغ عشرين سنتيمتراً. إضافة خليّة إلى القلب
    // تُبقي كل ما تحت البلاطة بين عقد مُسطَّحة.
    const half = size / 2 + TERRAIN_CELL;
