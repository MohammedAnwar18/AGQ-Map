...
out body geom qt;`;
    ...
    const importGeoJsonFile = (file) => {
        ...
        reader.onload = (evt) => {
            try {
                ...
                // Filter out features with null/invalid geometry
                parsedFeatures = parsedFeatures.filter(f =>
                    f && f.geometry && f.geometry.type && f.geometry.coordinates
                );

                if (parsedFeatures.length === 0) {
                    alert("⚠️ الملف فارغ أو لا يحتوي على معالم صالحة.");
                    return;
                }

                // Auto-detect dominant type for styling
                ...
                let geojson = { type: 'FeatureCollection', features: parsedFeatures };
                
                // Reproject coordinate systems (e.g. Web Mercator) to WGS84
                ...
                // ── Auto-zoom to imported data bounding box ──
                try {
                    const allLngs = [], allLats = [];
                    const collectCoords = (coords) => {
                        if (!coords) return;
                        if (typeof coords[0] === 'number') { allLngs.push(coords[0]); allLats.push(coords[1]); }
                        else coords.forEach(c => collectCoords(c));
                    };
                    geojson.features.forEach(f => collectCoords(f.geometry?.coordinates));
                    if (allLngs.length > 0) {
                        const bounds = [
                ...
                alert(`✅ تم استيراد ${parsedFeatures.length} معلم من "${file.name}" بنجاح!`);
            } catch (err) {
    ...
    // Collect all vertex handles (as GeoJSON Point features) from all visible layers
    const buildVertexHandlesGeoJson = (layers) => {
        const features = [];
        layers.forEach(layer => {
            if (layer.isVisible === false || !layer.data?.features) return;
            layer.data.features.forEach((f, fi) => {
                if (!f.geometry) return;
                const { type, coordinates } = f.geometry;
                const addV = (coord, extraProps) => {
                    if (!coord || coord.length < 2) return;
                    features.push({
                        type: 'Feature',
                        geometry: { type: 'Point', coordinates: [coord[0], coord[1]] },
                        properties: { lid: layer.id, fi: String(fi), ...extraProps }
                    });
                };
                if (type === 'Point') {
                    addV(coordinates, { gt: 'pt' });
                } else if (type === 'LineString') {
                    coordinates.forEach((c, vi) => addV(c, { gt: 'ls', vi: String(vi) }));
                } else if (type === 'Polygon') {
                    coordinates.forEach((ring, ri) =>
                        ring.forEach((c, vi) => addV(c, { gt: 'pg', ri: String(ri), vi: String(vi) }))
                    );
                } else if (type === 'MultiPolygon') {
                    coordinates.forEach((poly, pi) =>
                        poly.forEach((ring, ri) =>
                            ring.forEach((c, vi) => addV(c, { gt: 'mpg', pi: String(pi), ri: String(ri), vi: String(vi) }))
                        )
                    );
                } else if (type === 'MultiLineString') {
                    coordinates.forEach((line, li) =>
                        line.forEach((c, vi) => addV(c, { gt: 'mls', li: String(li), vi: String(vi) }))
                    );
                }
            });
        });
    ...
    // Apply a single vertex coordinate change to a geometry
    const applyVertexEdit = (geometry, props, newCoord) => {
        const geom = JSON.parse(JSON.stringify(geometry));
        const { gt } = props;
        const vi = parseInt(props.vi ?? 0);
        const ri = parseInt(props.ri ?? 0);
        const pi = parseInt(props.pi ?? 0);
        const li = parseInt(props.li ?? 0);

        if (gt === 'pt') {
            geom.coordinates = [newCoord[0], newCoord[1]];
        } else if (gt === 'ls') {
            geom.coordinates[vi] = newCoord;
        } else if (gt === 'pg') {
            const ring = geom.coordinates[ri];
            ring[vi] = newCoord;
            // Keep polygon closed: first == last
            if (vi === 0) ring[ring.length - 1] = newCoord;
            else if (vi === ring.length - 1) ring[0] = newCoord;
        } else if (gt === 'mpg') {
            const ring = geom.coordinates[pi][ri];
            ring[vi] = newCoord;
            if (vi === 0) ring[ring.length - 1] = newCoord;
            else if (vi === ring.length - 1) ring[0] = newCoord;
        } else if (gt === 'mls') {
            geom.coordinates[li][vi] = newCoord;
        }
        return geom;
    };
    ...
    const handleEditMouseDown = (e) => {
        ...
        // ── Priority 1: vertex handle ──
        const vHandle = rendered.find(f => f.layer.id === 'edit-vertex-handles');
        if (vHandle) {
            const props = vHandle.properties;
            const layerId = props.lid;
            const featureIdx = parseInt(props.fi);
            const targetLayer = geoLayers.find(l => l.id === layerId);
            if (!targetLayer?.data?.features?.[featureIdx]) return;
        ...
        const rawLayerId = myFeature.layer.id;
        const layerId = rawLayerId.replace(/^(poly-|line-|point-)/, '');
        const targetLayer = geoLayers.find(l => l.id === layerId);
        if (!targetLayer?.data?.features) return;

        // Match by geometry type (take first match)
        const clickedType = myFeature.geometry.type;
        const featureIdx = targetLayer.data.features.findIndex(f => f.geometry?.type === clickedType);
    ...
    const handleEditMouseMove = (e) => {
        ...
        if (editDragState.mode === 'vertex') {
            // Snap vertex directly to mouse cursor
            const newCoord = [e.lngLat.lng, e.lngLat.lat];
            const newGeom = applyVertexEdit(
                editDragState.originalGeometry,
                editDragState.vertexProps,
                newCoord
            );
        ...
        } else if (editDragState.mode === 'feature') {
            // Translate entire feature by delta from drag start
            const dLng = e.lngLat.lng - editDragState.startLngLat[0];
            const dLat = e.lngLat.lat - editDragState.startLngLat[1];
            const tc = (c) => [c[0] + dLng, c[1] + dLat];
            const tr = (ring) => ring.map(tc);
            const orig = editDragState.originalGeometry;
            let newCoords;
            if (orig.type === 'Point') newCoords = tc(orig.coordinates);
            else if (orig.type === 'LineString') newCoords = orig.coordinates.map(tc);
            else if (orig.type === 'Polygon') newCoords = orig.coordinates.map(tr);
            else if (orig.type === 'MultiPolygon') newCoords = orig.coordinates.map(p => p.map(tr));
            else if (orig.type === 'MultiLineString') newCoords = orig.coordinates.map(l => l.map(tc));
            else return;
            ...
            setGeoLayers(prev => prev.map(layer => {
                ...
                features[editDragState.featureIdx] = {
                    ...features[editDragState.featureIdx],
                    geometry: { ...orig, coordinates: newCoords }
                };
                return { ...layer, data: { ...layer.data, features } };
            }));
        }
    ...
    const vertexHandlesGeoJson = useMemo(() => {
        if (!editModeActive) return { type: 'FeatureCollection', features: [] };
        return buildVertexHandlesGeoJson(geoLayers);
    }, [geoLayers, editModeActive]);
                ...
                <section className="canvas">
                    <div className="map-container-inner">
                        ...
                        >
                            ...
                            {editModeActive && vertexHandlesGeoJson.features.length > 0 && (
                                <Source id="edit-vertex-source" type="geojson" data={vertexHandlesGeoJson}>
                                    <Layer
                                        id="edit-vertex-handles"
                                        type="circle"
                                        paint={{
                                            'circle-radius': 7.5,
                                            'circle-color': '#fbab15',
